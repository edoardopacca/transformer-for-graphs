...
The runs of Section~\ref{sec:plan} are completing in order. Below we report the first one that
is finished and analysed --- the dedicated trainable classifier (Section~\ref{sec:plan-barbell},
part~(b)). The base-model connectivity eval (per-block asymmetry, clique-size scaling,
oracle-agreement) and the training-density sweep are still running; their results, and the
overall matrix-powering-vs-DFS verdict, will be added as they land.
...
final curve hides a transient that is exactly the shape DFS predicts. Early in training
(Table~\ref{tab:clf_early}, Figure~\ref{fig:clf}, left, at step $2{,}000$) accuracy is high at
small cliques and \emph{falls at large ones}: on seed $1000$ it is $1.00$ up to $c=6$ but only
$0.65$ at $c=8$ and $0.55$ ($\approx$ chance) at $c=10$; seed $3000$ shows a milder version
($0.87$ at $c=10$), seed $4000$ milder still, and seed $2000$ is already flat. In other words the
larger, denser cliques are the \emph{last} to be mastered --- a bounded-traversal-like ordering
that the model eventually overcomes with more steps rather than a hard limit. The fixed-$c$ runs
corroborate the optimisation side of this (Figure~\ref{fig:clf}, right): although accuracy hits
$1.0$ immediately at every $c$, the early training \emph{loss} rises clearly with the clique
size ($\approx0.05$ at $c=3$, $\approx0.17$ at $c=6$, $\approx0.35$ at $c=10$), so a denser clique
is genuinely a bit harder to fit --- the ``density in optimisation'' effect DFS predicts ---
just not hard enough to delay reaching $100\%$.
...
\bottomrule
\end{tabular}
\caption{Classification accuracy \textbf{by clique size, early in training (step $2{,}000$)}, for
the four random-$c$ seeds ($n=20$, balanced test set, $200$ graphs/label/size). Bold marks the
cells near chance. The larger cliques are learned last (seed-dependently); by the end of training
every cell is $\approx1.00$ (last row of the figure), so this is a transient ordering, not a
wall.}
\label{tab:clf_early}
\end{table}
...
\begin{figure}[H]
    \centering
    \figorbox{runs/report5/report5_figs/classifier_by_clique_size.png}{1.0}
    \caption{Bridged-vs-split classifier, $n=20$, minimal trunk (mean-pool $+$ one logit).
    \emph{Left:} accuracy by clique size at step $2{,}000$, one line per random-$c$ seed; the
    larger cliques lag (seed-dependently), while the final accuracy is flat at $1.0$ for every
    seed (green dashed). \emph{Right:} fixed-clique-size runs --- the early ($2{,}000$-step)
    training loss grows with the clique size, even though accuracy is already $1.0$ at that point
    for every $c$.}
    \label{fig:clf}
\end{figure}

\paragraph{Two caveats, stated plainly.} First, training is \emph{noisy}: some seeds suffer
transient mid-run collapses to chance on the hardest cliques before settling (e.g.\ seed $4000$
dips on $c=10$ well past step $100{,}000$), so the clique-size ordering is a tendency of the
early dynamics, not a clean monotone law --- the familiar seed lottery of Reports~III--IV, here at
the level of which clique sizes are mastered when. Second, and more important for the headline
...
whether its errors carry the ordered ``start node fully resolved, far clique only partial''
asymmetry of a bounded traversal --- is what the base-model eval (Section~\ref{sec:plan-barbell}(a))
and the oracle comparison (Section~\ref{sec:plan-oracle}) decide, and those are still running.
