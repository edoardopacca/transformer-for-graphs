...
($\approx 10^9$ graphs), bf16. Appendix~D.1 specifies a \emph{RoBERTa}
implementation rather than the idealised A.1 transformer, so we run the
configuration at two points on the implementation-detail axis: a clean
\textbf{A.1-style} model (pre-LayerNorm, no dropout, default initialisation) and
a \textbf{RoBERTa-faithful} model (post-LayerNorm, dropout $0.1$, weight
initialisation $\mathcal{N}(0, 0.02)$). We treat these as two settings of the
same experiment, not as two experiments; as the results show, the conclusion
does not hinge on the distinction.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{runs/repro_paper_n20_roberta/fig1_reproduction.png}
    \caption{Figure~1, RoBERTa-faithful (unrestricted; mean over $3$ seeds, band
    is $\pm 1$ std, faint lines are individual seeds). One seed lifts 2Clique to
    $1.0$; the other two stay at zero, hence the wide band.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{runs/repro_paper_n20_roberta/fig7_reproduction.png}
        \caption{2Chain (paper Figure~7).}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{runs/repro_paper_n20_roberta/fig11_reproduction.png}
        \caption{2Clique (paper Figure~11).}
    \end{subfigure}
    \caption{The data lever, RoBERTa-faithful: exact-match accuracy on 2Chain
    (left) and 2Clique (right), unrestricted vs.\ restricted $D \le 9$ (mean over
    $3$ seeds, $\pm 1$ std band, individual seeds faint). The single generalising
    seed drives both the mean and the band; the two conditions are not cleanly
    separated.}
