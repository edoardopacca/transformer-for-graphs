...
read-out in one place; \S\ref{sec:cycles} tests one specific structural
claim the existing evidence suggests (that a path's open ends carry special
weight) by removing the ends entirely; \S\ref{sec:outlook} lists the concrete
next steps this framing calls for --- comparing the raw per-layer embeddings
$h^{(1)}, h^{(2)}$ directly (not only derived quantities) to see whether a
two-component ``block'' structure is visible layer by layer, inspecting the
attention output added to the residual stream directly, and a revised
node-to-node contribution measure that differentiates with respect to the
\emph{input} adjacency rather than the post-read-in embedding.
...
\subsection{Node-to-node contribution}
...
anything (\S\ref{sec:recap}). \emph{This measure will be revisited once the
revised, adjacency-based contribution of \S\ref{sec:outlook} is defined; the
figures here use the same $C_{ik}$ as every result since Report~VII.}
...
battery: the behavioural sweep, and the node-to-node contribution leak
fraction, using the \emph{same} exact-Jacobian contribution measure as
\S\ref{sec:battery-contrib} (the revised, adjacency-based measure of
\S\ref{sec:outlook} is deliberately not used yet, so this result is not
entangled with that separate open question).
...
comparison against Figure~\ref{fig:r8leak}, and the verdict once the job
returns.]}

\section{Outlook}
\label{sec:outlook}

Three concrete next steps follow directly from the framing of
\S\ref{sec:question}, none implemented yet.

\begin{itemize}\setlength\itemsep{0.15em}
  \item \textbf{A revised node-to-node contribution measure.} Every
        contribution result since Report~VII (including
        \S\ref{sec:battery-contrib} above) differentiates the final
        embedding $h_i^{(2)}$ with respect to the \emph{post-read-in}
        embedding $h_k^{(0)}$. The concern: $h_k^{(0)}$ is already a learned
        transformation of the input, so by the time it is used as the
        differentiation variable it may no longer be as tightly tied to
        ``node $k$ specifically'' as the raw input row is. The planned fix
        differentiates with respect to the input adjacency $A'$ itself ---
        concretely, how the final embedding of node $i$ changes if a single
        edge is flipped from $0$ to $1$ (or the reverse) in $A'$, rather than
        through the already-processed $h^{(0)}$. The exact experimental
        design is still being worked out and will be specified in a
        follow-up before it is implemented.
