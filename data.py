...
def generate_stitched_theta_graph_truncated(n: int, sizes: tuple[int, ...] = (20, 20, 20),
    ...
    hub. ``n_isolated`` (2026-09-01) pads the canvas with that many extra degree-0 singleton
    nodes when ``sum(sizes) < n``, exactly as in generate_stitched_theta_graph -- e.g.
    sizes=(19,20,19), n_isolated=2 at n=60 gives the same 19/20/20-route hub pair as
    theta_19_20_19, now with routes A and C also severed. No self-loops; node order NOT
    permuted (permute at the call site, as in every other generator here)."""
    if len(sizes) != 3:
        raise ValueError(f"expected exactly 3 components (theta needs a middle one), got {sizes}")
    adj = generate_stitched_theta_graph(n, sizes, n_isolated=n_isolated).copy()
    a0, a1 = 0, sizes[0] - 1
    c0, c1 = sizes[0] + sizes[1], sizes[0] + sizes[1] + sizes[2] - 1
