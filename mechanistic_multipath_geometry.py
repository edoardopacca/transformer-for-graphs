...
def eval_k(model, dev, n, k, path_len, term_deg, rng, n_graphs):
    ...
    G2 = _cosine_batch(h2_base).mean(0)                 # [n,n] mean cosine, base order
...
def make_figures(results, out_dir):
    ...
    ax.plot(ks, [r["term_connect"] for r in results], "-o", color=BLUE, ms=8, lw=2)
...
def main():
    ...
    for k in args.ks:
        ...
        results.append(r)
        print(f"  k={k}: term_connect={r['term_connect']:.3f} reach_route={r['reach_route']:.3f} "
              f"pred_pos={r['pred_positive_rate']:.3f}", flush=True)
        for cat in CATEGORIES:
            g = r["geometry_H2"][cat]
            if g is not None:
                print(f"    {cat:16s} mean_cos={g['mean_cosine_H2']:+.3f} "
                      f"logit={g['implied_logit']:+.3f} connected={g['implied_connected']}")
